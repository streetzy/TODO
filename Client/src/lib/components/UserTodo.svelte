<script lang="ts">
  ...
  async function addTodo() {
    await fetch(URL + "todo", {
      ...
      method: "POST",
      body: JSON.stringify({
        todoName: (document.getElementById("todo-name") as HTMLInputElement)
          .value,
        todoDescription: (
          document.getElementById("todo-description") as HTMLInputElement
        ).value,
        todoDeadline: (document.getElementById("todo-date") as HTMLInputElement)
          .value,
  ...
  async function editTodo(todoId: string) {
    await fetch(URL + "todo/" + todoId, {
      ...
      body: JSON.stringify({
        ...
        changeName: (
          document.getElementById("edit-todo-name") as HTMLInputElement
        ).value,
        changeDescription: (
          document.getElementById("edit-todo-description") as HTMLInputElement
        ).value,
        changeDeadline: (
          document.getElementById("edit-todo-date") as HTMLInputElement
        ).value,
      }),
    });
  ...
  async function moveTo(todoId: string, where: string) {
    // where being the working-on tab or the done tab
    await fetch(URL + "todo/" + todoId, {
      headers: {
        Authorization: `${accessToken}`,
        "Content-Type": "application/json",
  ...
  async function addInvite() {
    //functions similar to steam, "send me ur code (in this case the userId)" to test sampleuserID: 66265d5a13fec2e587d51488
    await fetch(URL + "group/" + currentGroupId + "/invite", {
      ...
      body: JSON.stringify({
        userId: (
          document.getElementById("requested-user-id") as HTMLInputElement
        ).value,
      }),
    });
...
<main>
  {#if showAddInviteWindow}
    <!--reused CSS properties, that's why naming might seem confusing-->
  ...
  {#if isInInviteView}
    <div class="invite-view-content">
      <nav>
        <button class="button-text" on:click={() => showInviteView()}
          >TODOs</button
        >
        {#if !acceptingInvites}
          <button class="button-text" on:click={() => showAddInviteView()}
            >ADD</button
          >
        {/if}
        <button class="button-text" on:click={() => acceptingInviteView()}
          >{acceptingInvites ? "GROUP VIEW" : "USER VIEW"}</button
        >
        <h1 class="username">
          {acceptingInvites ? requestedUserInfo.userName : currentGroupName}
        </h1>
      </nav>
          ...
          {#each userInvites as userInvite}
            <div class="shown-invite">
              ...
              <div class="invite-buttons">
                <button
                  class="shown-invite-request"
                  on:click={() => acceptInvite(userInvite.inviteId)}
                  >&#x2713;</button
                >
                <!--Checkmark-->
                <button
                  class="shown-invite-request"
                  on:click={() => rejectInvite(userInvite.inviteId)}>X</button
                >
              </div>
            </div>
          {/each}
        {:else}
          ...
          {#each groupInvites as groupInvite}
            <div class="shown-invite">
              ...
              <button
                class="shown-invite-cancel"
                on:click={() => cancelInvite(groupInvite.inviteId)}
                >CANCEL</button
              >
            </div>
          {/each}
  ...
  {/if}
  {#if editingTodoWindow}
    <!--haha same chunk of code but too dumb to remake this in a smart away whilst utilizing Svelte-->
    <div class="adding-todo-window">
      <div class="input">
        <h1 class="add-todo-header">Name Of Todo</h1>
        <input
          class="todo-input"
          value={currentTodo.todoName}
          type="text"
          id="edit-todo-name"
        />
      </div>
      ...
      <div class="input">
        ...
        <input
          class="todo-input"
          value={currentTodo.deadline ? currentTodo.deadline : ""}
          type="date"
          id="edit-todo-date"
          min="1970-01-01"
        />
      </div>
      <button class="add-button" on:click={() => editTodo(currentTodoId)}
        >SUBMIT</button
      >
      <button class="add-button" on:click={() => showEditingTodoWindow()}
        >CANCEL</button
      >
    </div>
  {/if}
  {#if addingTodoWindow}
    <div class="adding-todo-window">
      ...
      <button class="add-button" on:click={() => showAddingTodoWindow()}
        >CANCEL</button
      >
    </div>
  {/if}
  {#if groupPopUpFlag && userGroupData != null && userGroupData != undefined}
    <div class="group-pop-up-container">
      <div class="group-pop-up-header">
        ...
        <button class="add-group" on:click={() => showAddingGroupWindow()}
          >ADD GROUP</button
        >
      </div>
      <div class="group-display">
        {#each userGroupData as userGroup}
          <button
            class="select-group"
            on:click={() => selectGroup(userGroup.groupId, userGroup.groupName)}
            >{userGroup.groupName}</button
          >
        {/each}
      </div>
    </div>
    {#if addingGroup}
      <div class="groupDetails">
        ...
        <div class="groupInput">
          ...
          >
          <button class="inputButton" on:click={() => showAddingGroupWindow()}
            >GROUP VIEW</button
          >
        </div>
      </div>
  ...
  {:else}
    <nav>
      <button class="calendar-view" on:click={() => swapToCalendar()}
        ><h1 class="button-text">CALENDAR VIEW</h1></button
      >
      <button class="group-view" on:click={() => showGroupPopUp()}
        ><h1 class="button-text">GROUP VIEW</h1></button
      >
      <button class="invite-view" on:click={() => showInviteView()}
        ><h1 class="button-text">INVITES</h1></button
      >
      <div class="dropdown">
    ...
    <div class="content">
      <div class="todo to-do-tab">
        <div class="header">
          <h1>TO-DO</h1>
          <button class="addTodoButton" on:click={() => showAddingTodoWindow()}
            >+</button
          >
        </div>
          ...
          {#each todoItems as todo}
            {#if todo.status == "to-do"}
              <div class="todo-item">
                ...
                <div class="buttons">
                  <button
                    on:click={() => moveTo(`${todo.todoId}`, "working-on")}
                    class="to-working-on"
                  ></button>
                  <button
                    on:click={() => showInformation(`${todo.todoId}`)}
                    class="todo-info"
                  ></button>
                  <button
          ...
          {#each todoItems as todo}
            {#if todo.status == "working-on"}
              <div class="todo-item">
                ...
                <div class="buttons">
                  <button
                    on:click={() => moveTo(`${todo.todoId}`, "done")}
                    class="to-working-on"
                  ></button>
                  <button
                    on:click={() => showInformation(`${todo.todoId}`)}
