<script lang="ts">
  ...
  let orderedTodos: {
    [date: string]: { todoName: string; status: string; todoId: string }[];
  } = {};
  ...
  function orderTodos() {
    // clear the orderedTodos from previous entries, then go through todoItems, use key that we sort the deadline with
  ...
  function daysInMonth(month: number) {
    // get the amount of days in the month, then sort that into an array of arrays, where each has 7 (or less for the last week) days
    const firstDayOfMonth = firstDayIndexInMonth(
      months.findIndex((month) => month == currentMonth)
    ); // returns an index, we then look it up in the days array and
  ...
  function changeMonth(previousFlag: boolean) {
    ...
    previousFlag
      ? (currentMonth =
          months[months.findIndex((month) => month == currentMonth) - 1])
      : (currentMonth =
          months[months.findIndex((month) => month == currentMonth) + 1]);
  }
...
<main>
  <nav>
    <div class="nav-child">
      <button class="return" on:click={() => toggleCalendarView()}
        ><h1 class="button-text">RETURN</h1></button
      >
    </div>
    <div class="nav-child">
  ...
  <div class="calendar-grid">
    {#if showDayInfoWindow}
      <div class="day-info-container">
        ...
        <button on:click={() => toggleDayInfoWindow()}
    ...
    {:else}
      ...
      <div class="calendar-content">
        {#each calendarDays as week}
          <!--Go through the weeks in the calendarDays array, then go through the days of every week,
            ...
            <div class={day == null ? "null-day" : "normal-day"}>
              ...
              >
                ...
                <div class="day-content">
                  {orderedTodos[
                    `${currentYear}-${months.findIndex((month) => month == currentMonth) + 1}-${day}`
                  ]?.length || 0}
                </div>
              </button>
            </div>
